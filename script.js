...
function addPartRow(data = {}) {
  ...
  row.querySelector('.part-rot').value = String(data.rotation ?? true);
...
function explodeParts(parts) {
  ...
  // 4) Dividir o espaço remanescente em sub-retângulos (direita e abaixo), considerando o kerf.
...
function tryPlaceOnSheet(sheet, item, kerf) {
  ...
  sheet.freeRects.forEach((rect, rectIndex) => {
    ...
    candidates.forEach((candidate) => {
      if (candidate.w <= rect.w && candidate.h <= rect.h) {
        const score = rect.w * rect.h - candidate.w * candidate.h;
        if (!best || score < best.score) {
          best = { rectIndex, rect, candidate, score };
        }
      }
    });
  });
  ...
  splitFreeRect(sheet.freeRects, rect, placement, kerf);
  pruneFreeRects(sheet.freeRects);

  return placement;
}

function splitFreeRect(freeRects, rect, placement, kerf) {
  const rightW = rect.w - placement.width - kerf;
  const bottomH = rect.h - placement.height - kerf;

  if (rightW > 0) {
    freeRects.push({
      x: rect.x + placement.width + kerf,
      y: rect.y,
      w: rightW,
      h: rect.h
    });
  ...
  if (bottomH > 0) {
    freeRects.push({
      x: rect.x,
      y: rect.y + placement.height + kerf,
      w: placement.width,
      h: bottomH
    });
  }
